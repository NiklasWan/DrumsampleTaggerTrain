...
from musicnn_drumsamp_keras import configuration as config
...
def extractor(file_name, model='MSD_musicnn', input_length=3, input_overlap=False, extract_features=False):
    ...
    Data format: string.
    Options: 'MTT_musicnn', 'MTT_vgg', 'MSD_musicnn', 'MSD_musicnn_big', 'DRUMSAMP_MTT_musicnn' or 'MSD_vgg'.
    ...
    try:
        if 'DRUMSAMP' in model:
            keras_model = tf.keras.models.load_model('./musicnn_drumsamp_keras/keras_checkpoints/{}.hdf5'.format(model))
        else:
            keras_model = tf.keras.models.load_model('./musicnn_drumsamp_keras/keras_checkpoints/{}.h5'.format(model))
    except:
        raise ValueError('Unknown model')

    # select labels
    if 'DRUMSAMP' in model:
        labels = config.DRUMSAMP_LABELS
    elif 'MTT' in model:
        ...
        if 'vgg' in model:
            ...
            if 'DRUMSAMP' in model:
                intermediate_layer_model_penultimate = tf.keras.Model(keras_model.input,
                                                                    keras_model.get_layer('dense_153').output)
            else:
                intermediate_layer_model_penultimate = tf.keras.Model(keras_model.input,
